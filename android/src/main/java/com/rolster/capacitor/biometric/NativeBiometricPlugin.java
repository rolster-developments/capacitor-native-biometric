...
public class NativeBiometricPlugin extends Plugin {
  ...
  public void isAvailable(PluginCall call) {
    ...
    // > "Developers that wish to check for the presence of a PIN, pattern, or
    // password on these versions should instead use isDeviceSecure."
    // @see
    ...
    if (!isAvailable) {
      // BiometricManager Error Constants use the same values as BiometricPrompt's
      // Constants. So we can reuse our
  ...
  public void verifyIdentity(final PluginCall call) {
    Intent intent = new Intent(getContext(), NativeBiometricActivity.class);
    ...
    if (username != null && password != null && server != null) {
      try {
        SharedPreferences.Editor editor = getContext()
            .getSharedPreferences(NATIVE_BIOMETRIC_SHARED_PREFERENCES, Context.MODE_PRIVATE).edit();
  ...
  public void getCredentials(final PluginCall call) {
    ...
    SharedPreferences sharedPreferences = getContext().getSharedPreferences(NATIVE_BIOMETRIC_SHARED_PREFERENCES,
  ...
  private Key generateKey(String KEY_ALIAS, boolean isStrongBoxBacked)
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      ...
      KeyGenParameterSpec.Builder paramBuilder = new KeyGenParameterSpec.Builder(KEY_ALIAS,
  ...
  private Key getAESKey(String KEY_ALIAS) throws CertificateException, NoSuchPaddingException, InvalidKeyException,
  ...
  private byte[] rsaEncrypt(byte[] secret, String KEY_ALIAS) throws CertificateException, NoSuchAlgorithmException,
      KeyStoreException, IOException, UnrecoverableEntryException, NoSuchProviderException,
  ...
  private byte[] rsaDecrypt(byte[] encrypted, String KEY_ALIAS) throws UnrecoverableEntryException,
